...
*   ** Python (`main.py`)**: 負責前期的環境調教（Robust Environment Configuration）與自動化數據鏈結。使用 **Pandas** 進行數據讀取與初步特徵檢查（Data Inspection），並建立自動化防錯機制（Try-Catch block）。
*   ** SQL (`medical.sql`)**: 負責核心數據指標的撈取與高級分析（Advanced Querying）。分析患者年齡、預約週期、簡訊通知（SMS）與赴約率之間的關聯性。
*   ** Tableau (`medical_tableau_analysis.twb`)**: 將分析結果轉化為動態商業儀表板（Interactive Dashboard），提供決策者一目了然的視覺化洞察。
